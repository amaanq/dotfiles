  ...
  config = lib.mkIf isPower64 {
    ...
    nixpkgs.overlays = [
      (final: prev:
        let
          ...
          mkSysCrateOverride =
            ...
            pkg.overrideAttrs (old: {
              ...
              '' + cargoCcRouterPreBuild + pkgConfigRouterPreBuild;
            });
        ...
        in
        {
        # Bump rust to 1.95.0 for the powerpc64 callconv fix (PR #150468);
        # without it ppc64 BE ELFv2 corrupts every extern "C" aggregate return.
        # Drop once nixpkgs unstable ships 1_95.nix.
        rust_1_94 =
          let
            llvmSharedFor =
              pkgSet:
        ...
        # Override the alias so the final cross rustc gets our patches while
        # the bootstrap chain stays vanilla.
        ...
        bcachefs-tools = (prev.bcachefs-tools.overrideAttrs (old: {
          ...
          # (0x80081272). PowerPC, MIPS, SPARC use the 3-bit dir / 13-bit
          # size layout and want 0x40081272 instead — without this, mkfs
          # sees ppc64 block devices as 0 bytes and refuses to format.
          # Drop once https://… (Kent's tree) merges.
          ...
          _: lprev: {
            llvm = lprev.llvm.overrideAttrs (old: {
              postConfigure = (old.postConfigure or "") + ''
                find . -name "*.ninja" -exec sed -i 's/-O3/-O2/g' {} +
              '';
              preInstall = (old.preInstall or "") + lib.optionalString (
