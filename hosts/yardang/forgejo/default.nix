...
let
  ...
  forgejo' = (pkgs.callPackage (self + /packages/forgejo.nix) { git = pkgs.gitMinimal; }).overrideAttrs (old: {
...
in
{
  ...
  # The forgejo NixOS module hardcodes `pkgs.git` (full, perl-enabled) in the
  # systemd unit's PATH regardless of cfg.package's git arg. Override to
  # substitute gitMinimal and keep perl out of the closure.
